...
import { Truck, Box, MapPin, AlertCircle, ShieldCheck, MessageSquare, LayoutDashboard, Search } from 'lucide-react';
...
const data = [
  ...
  { name: 'Karadeniz', sevkiyat: 3900, anomali: true }, // Anomali vurgusu
];
...
export default function SmartOpsDashboard() {
  ...
  const fullText = "Merhaba , Türkiye genelindeki lojistik modelleri %94 doğrulukla analiz edildi. Karadeniz hattındaki hava durumu verileri derin öğrenme modeline entegre edildi. 3 araçta rota sapması (anomali) saptandı.";

  useEffect(() => {
    setIsReady(true);
    // Streaming Effect (Karakter karakter yazma)
    let index = 0;
    setAiText(""); 
    const interval = setInterval(() => {
      setAiText((prev) => prev + fullText[index]);
      index++;
      if (index >= fullText.length - 1) clearInterval(interval);
    }, 30);
    return () => clearInterval(interval);
  }, [role]); // Rol değişince AI tekrar yazsın
          ...
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              <div className="bg-slate-950 rounded-[48px] p-10 text-white flex flex-col shadow-2xl border-b-[16px] border-blue-600">
                ...
                <div className="flex-1 bg-white/5 rounded-[32px] p-8 mb-8 font-mono text-[12px] leading-relaxed text-blue-100 border border-white/5 overflow-y-auto">
                   {aiText}
                   <span className="inline-block w-2 h-4 bg-blue-500 ml-1 animate-bounce"></span>
                </div>
                <div className="relative">
                  <input type="text" placeholder="AI'dan rapor iste..." className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-xs focus:outline-none focus:ring-2 ring-blue-500 transition-all" />
                  <button className="absolute right-3 top-3 p-2 bg-blue-600 rounded-xl hover:scale-105 transition-transform">
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            <div className="bg-white p-12 rounded-[48px] border-2 border-dashed border-slate-200 text-center space-y-6">
              ...
              <div className="flex gap-4 max-w-lg mx-auto pt-6">
                <input type="text" placeholder="Örn: TR-123456" className="flex-1 bg-slate-100 border-none rounded-3xl p-6 text-lg font-bold outline-none focus:ring-4 ring-blue-100 transition-all" />
