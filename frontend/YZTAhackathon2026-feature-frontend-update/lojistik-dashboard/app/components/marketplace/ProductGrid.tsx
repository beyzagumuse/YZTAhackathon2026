...
export default function ProductGrid({ onAddToCart }: { onAddToCart: (product: any) => void }) {
  ...
  useEffect(() => {
    // API isteği atıyoruz (Sonundaki slash'e dikkat)
    fetch("http://localhost:8000/products/")
      .then(async (res) => {
        const data = await res.json();
        
        // Eğer sunucu 200 OK dönmediyse hatayı fırlat
        if (!res.ok) {
           throw new Error(data.detail || "API'den geçerli bir yanıt alınamadı.");
        }
        return data;
      })
      .then(data => {
        ...
        if (!Array.isArray(data)) {
          ...
          throw new Error("Sunucudan ürün listesi yerine farklı bir veri formatı geldi.");
        }

        const formatted = data.map((p: any) => ({
          id: p.id,
          name: p.name,
          coop: "Yöresel Kooperatif", 
          price: p.price,
          stock: 10,
  ...
  if (loading) return <div className="p-12 text-center text-slate-400 font-bold uppercase tracking-widest">Ürünler Yükleniyor...</div>;

  // Hata varsa veya ürün yoksa gösterilecek ekran
  if (errorMsg || products.length === 0) return (
    <div className="p-12 mt-16 text-center border-2 border-dashed border-rose-100 rounded-[32px] bg-rose-50">
       ...
       <p className="text-rose-600 font-bold uppercase tracking-widest text-sm">Ürünler Listelenemedi</p>
       <p className="text-rose-400 text-xs mt-2">{errorMsg || "Sistemde kayıtlı ürün bulunmuyor."}</p>
    </div>
  );

  return (
    <section className="mt-16 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <h2 className="text-3xl font-black tracking-tighter text-slate-900 mb-8">Taze Ürünler</h2>
